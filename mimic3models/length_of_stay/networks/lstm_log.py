...
class Network(nn_utils.BaseNetwork):
    ...
    def __init__(self, dim, mode, l2, l1, batch_norm, dropout,
        ...
        network = layers.DenseLayer(incoming=network, num_units=1,
        ...
        self.loss_mse = squared_error(self.prediction,
    ...
    def process_input(self, data_raw):
        return (nn_utils.pad_zeros(data_raw[0]).astype(np.float32),
                map(len, data_raw[0]), 
                np.log(1.0 + np.array(data_raw[1]).astype(np.float32)),
                np.array(data_raw[1]).astype(np.float32))
    ...
    def predict(self, data):
        ...
        return [np.exp(ret[0]) - 1] + list(ret[1:])