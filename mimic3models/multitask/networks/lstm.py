...
class Network(nn_utils.BaseNetwork):
    ...
    def __init__(self, train_raw, test_raw, dim, mode, l2, l1,
        ...
        los_network = layers.InputLayer((None, None, dim), input_var=lstm_output)
        los_network = layers.ReshapeLayer(los_network, (-1, dim))
        los_network = layers.DenseLayer(incoming=los_network, num_units=nbins,
                                        nonlinearity=softmax)
        los_network = layers.ReshapeLayer(los_network, (lstm_output.shape[0], -1, nbins))
        self.los_prediction = layers.get_output(los_network)
        self.los_det_prediction = layers.get_output(los_network, deterministic=True)
        self.params += layers.get_all_params(los_network, trainable=True)
        self.reg_params += layers.get_all_params(los_network, regularizable=True)
        los_prediction_reshaped = self.los_prediction.reshape((lstm_output.shape[0] * lstm_output.shape[1], nbins))
        los_label_reshaped = self.los_label.reshape((lstm_output.shape[0] * lstm_output.shape[1],))
        self.los_loss = categorical_crossentropy(los_prediction_reshaped,
                                                 los_label_reshaped).reshape((lstm_output.shape[0], lstm_output.shape[1]))
        self.los_loss = (self.los_mask * self.los_loss).mean(axis=1).mean(axis=0)
    ...
    def step(self, mode):
        ...
        return {"ihm_prediction": ret[0],
                "los_prediction": self.get_estimates(ret[1]),
                "ph_prediction": ret[2],
